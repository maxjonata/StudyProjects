...
A função retornará o número de remoções realizadas.*/
{
	...
	/*Obs: está sendo feito separadamente, pois a remoção no início da lista
	não poderá ser realizada com aquela ideia de dois ponteiros, um sendo a
	'sombra' do outro.*/
	...
	/*se a lista não estiver vazia. Afinal, todos os elementos podem ter sido
	excluídos no 'while' anterior*/
	if (*L)
	{	
		pre=*L;   		//pre apontando para o primeiro nó da lista
		pos=pre->prox;	//pos apontando para o segundo nó
		...
		while (pos)   //ou pos!=NULL
		{
			...
			if(pos->valor==numero)
			{				
				pre->prox=pos->prox;  //faz o nó anterior apontar para o próximo do elemento a ser removido
				rem++;
				free(pos);            //remove o elemento que contém 'numero'
				pos = pre->prox;     //reposiciona o 'pos', pois tem que continuar a busca até o último elemento da lista				
			}
			else
			{
				//se não for encontrado, andar com os dois ponteiros
				pre = pos;
				pos=pos->prox;
			}
        }
